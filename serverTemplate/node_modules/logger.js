...
module.exports = proto(/*MongoObject,*/ function() {
    ...
    var script, port
    this.initialize = function(scriptFilename, serverPort) {
        if(!initialized) {
            initialized = true
    ...
    this.logInContext = function(context, script, machineName, port, prio, msg/*, data, ...*/) {
        var log = {}
        var data = trimArgs(arguments).slice(6);
    ...
    this.log = function(prio, msg/*, data, ...*/) {
        var args = [this.context, script, machineName, port].concat(trimArgs(arguments))
    ...
    this.logLevel = function(level, args) {
        this.log.apply(this, [level].concat(trimArgs(args)))
    }
})
